<?php
/**
 * server_user_sftp.php - Acces SFTP/SSH par utilisateur distant (page dediee).
 * Separe de la gestion sudo (cf server_user_sudo.php) pour la clarte.
...
// Helper : case a cocher + explication sous le champ
function sftpToggle($id, $name, $label, $help, $checked) {
    $c = $checked ? 'checked' : '';
    echo '<div class="py-2 border-b border-gray-100 dark:border-gray-700/60">';
    echo '<label class="flex items-center gap-2 text-sm font-medium"><input type="checkbox" name="'.$name.'" id="'.$id.'" '.$c.' class="form-checkbox"><span>'.htmlspecialchars($label).'</span></label>';
    echo '<p class="text-xs text-gray-500 dark:text-gray-400 mt-1 ml-6">'.htmlspecialchars($help).'</p>';
    echo '</div>';
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title><?= t('sftppol.title') ?> - <?= $appName ?></title>
</head>
...
<div class="px-6 py-6 max-w-screen-xl mx-auto">
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-100"><?= t('sftppol.title') ?></h1>
    ...
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-xl p-4 mb-6">
        <p class="text-sm font-semibold text-blue-800 dark:text-blue-300 mb-1">💡 <?= t('sftppol.intro_title') ?></p>
        <p class="text-sm text-blue-900/80 dark:text-blue-200/80"><?= t('sftppol.intro') ?></p>
    </div>
...
<?php if ($selectedId && $selectedUserId): ?>
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6">
        <h2 class="text-base font-semibold mb-4"><?= t('sftppol.options') ?></h2>
        <form id="sftp-form" class="space-y-1">
            <?php
            sftpToggle('sftp-only', 'sftp_only', t('sftppol.f_sftp_only'), t('sftppol.h_sftp_only'), $existingSftp['sftp_only'] ?? false);
            ?>
            <div class="py-2 border-b border-gray-100 dark:border-gray-700/60">
                <label class="block text-sm font-medium mb-1"><?= t('sftppol.f_chroot') ?></label>
                <input type="text" name="chroot_dir" id="sftp-chroot" placeholder="/srv/sftp/jean" value="<?= htmlspecialchars($existingSftp['chroot_dir'] ?? '') ?>" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 font-mono text-xs">
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1"><?= t('sftppol.h_chroot') ?></p>
            </div>
            <div class="py-2 border-b border-gray-100 dark:border-gray-700/60">
                <label class="block text-sm font-medium mb-1"><?= t('sftppol.f_working') ?></label>
                ...
                <p class="text-xs text-gray-500 dark:text-gray-400 mt-1"><?= t('sftppol.h_working') ?></p>
            </div>
            <?php
            sftpToggle('sftp-pw', 'allow_password_auth', t('sftppol.f_password'), t('sftppol.h_password'), $existingSftp['allow_password_auth'] ?? true);
            sftpToggle('sftp-tcp', 'allow_tcp_forwarding', t('sftppol.f_tcp'), t('sftppol.h_tcp'), $existingSftp['allow_tcp_forwarding'] ?? true);
            sftpToggle('sftp-agent', 'allow_agent_forwarding', t('sftppol.f_agent'), t('sftppol.h_agent'), $existingSftp['allow_agent_forwarding'] ?? true);
            sftpToggle('sftp-x11', 'x11_forwarding', t('sftppol.f_x11'), t('sftppol.h_x11'), $existingSftp['x11_forwarding'] ?? false);
            ?>
        ...
        <?php if ($existingSftp): ?>
        <p class="text-xs text-gray-400 mt-4"><?= t('policies.last_deployed') ?>: <?= $existingSftp['last_deployed_at'] ?? 'jamais' ?>
            (<?= $existingSftp['enabled'] ? '<span class="text-green-600">'.t('policies.status_enabled').'</span>' : '<span class="text-gray-500">'.t('policies.status_disabled').'</span>' ?>)</p>
        <?php endif; ?>
