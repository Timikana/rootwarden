...
## 8. ⛔ LE DÉFAUT EST NATIF — mesuré au `blame` le 2026-09-09

Mon §1 disait *« le commentaire ratifie un chiffrement qui n'existe pas »*. Il faut le
dater, et la date change la classe du défaut :

    2129a2cf3  2026-04-11 18:30  :2464   # Chiffrer le token Telegraf si fourni
    2129a2cf3  2026-04-11 18:30  :2465   telegraf_token = data.get(…)          BRUT
    2129a2cf3  2026-04-11 18:30  :2466   if telegraf_token == '********':      garde de MASQUE
    2129a2cf3  2026-04-11 18:30  :717    if psk_value and psk_value != '…':    garde de MASQUE
    2129a2cf3  2026-04-11 18:30  :718    psk_encrypted = enc.encrypt_password(…)  CHIFFRE
    statut du fichier dans ce commit : A (creation)
    TEMOIN : :369 rend 70aff91e7 (2026-08-22) — 19 empreintes dans le fichier,
             le `blame` discrimine bien

> **Les deux secrets sont nés dans le même geste, à la même minute** — l'un chiffré,
> l'autre pas, **avec un commentaire affirmant que le mauvais était bon.**

### 8.1 Et l'omission est LOCALE, pas un oubli général

C'est le point que ni le premier relevé ni sa correction n'avaient : **les deux secrets
reçoivent le MÊME traitement du masque `'********'`** (`:717` pour le PSK, `:2466` pour
le jeton), écrit dans le même commit. L'auteur a donc traité *attentivement* la même
préoccupation pour les deux, et n'a chiffré qu'un seul.

> **Ce n'est pas « le jeton a été oublié ».** Il a été suivi sur une propriété et pas
> sur l'autre, dans le même souffle. *L'omission est locale au chiffrement.*

### 8.2 La conséquence de méthode, plus large que ce cas

Il n'y a **jamais eu** de moment où chiffrer le jeton aurait été un rattrapage : il n'a
jamais suivi le PSK.

> **Un défaut NATIF est invisible à tout instrument qui cherche une DÉRIVE.** Ni
> bissection, ni « qu'est-ce qui a changé », ni chasse à la régression n'y mènent — et
> c'est la classe entière d'outils qu'on emploie par réflexe sur un fichier de cinq mois.

### 8.3 Le témoin consolidé
...
⚠ **Un compte par sous-chaîne rendait `decrypt_password( = 4`** en agrégeant deux
fonctions distinctes. *`0 sur 4` se lit comme une anomalie, `0 sur 2` comme une
possibilité* : le même zéro, deux forces de conviction. **Le témoin et la mesure ne se
contrôlent pas séparément — c'est leur RAPPORT qui porte la conclusion.**

---

## 9. Note de traçabilité

Ce document a d'abord été committé le 2026-09-09 à 09:15 (`5db6fd49`) et a atterri par
accident sur `security/garde-socle-avertissement`, l'arbre partagé ayant été basculé
pendant l'écriture. **La version présente ici est la version échouée, augmentée du §8.**
Si cette branche est fusionnée un jour, le chemin entrera en conflit — *un conflit
visible, préféré à un document qui n'existe nulle part d'atteignable.*
