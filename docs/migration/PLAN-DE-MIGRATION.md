...
| LOT de tests E2E | **UN LOT COMPLET A TOURNÉ le 2026-08-28 et s'est TERMINÉ à 16:35 CEST** — lancé par la session 7, **PID 690188**, ~2 h 30. ⚠ **Le verdict n'est PAS connu du Lead** : `kill -0 690188` rend « mort », et *la disparition du processus n'est pas plus un verdict que ne l'était son code de sortie.* Le journal appartient à la session 7, qui rend exécutions / PASS / FAIL et, pour chaque FAIL, s'il est une régression. **Ligne de base à battre, 2026-08-27 : 150 exécutions · 2282 PASS · 3 FAIL · 2 h 44**, dont **aucun n'était une régression de l'application** (deux venaient des suites, un est E-90). **Le gel des conteneurs TIENT jusqu'à ce verdict** : si le LOT s'est arrêté sur un incident plutôt qu'à sa fin, une charge posée maintenant effacerait la trace. Remesure : `./scripts/rejouer-lot.sh`, et **attendre un PID enregistré** — `pgrep -f "[r]ejouer-lot"` a rendu **trois** PID dont le shell qui posait la question, la classe de caractères est nécessaire mais **pas suffisante**.
