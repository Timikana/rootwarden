...
| | |
|---|---|
| entrées de menu portées | **23 sur 33**, et le total se RECONSTITUE — mesuré le 2026-08-26 en faisant lire `Navigation::SECTIONS` **par PHP lui-même** : `23 route + 10 legacy + 0 ni l'un ni l'autre = 33` (D9a, D9b et B1 ont fait basculer `sudo_policies`, `sftp_policies` et `bashrc`). Un premier comptage à l'expression régulière avait rendu 32, en manquant `wazuh` — voir §8. Restent en `legacy` : `iptables`, `fail2ban`, `services`, `ssh_audit`, **`wazuh`**, `groups`, `remote_users`, `platform_key`, `documentation`, `api_docs` |
...
| 7 | `services/` | 631 (2 fichiers) | 1 | **INVENTORIÉ `v1.37.92` — `MODULE-SERVICES.md`**, trois sous-lots S1 à S3, **S1 CARACTÉRISÉ `v1.37.93`** (16 legacy, 0 FAIL). **E-149 : les huit routes n'ont NI rôle NI permission — seule la page est gardée.** Réel dans le code, non exploitable par aucun compte existant : à un `UPDATE` de l'être |
