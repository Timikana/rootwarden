...
| LOT de tests E2E | **NOUVELLE LIGNE DE BASE, 2026-08-28 : 153 exécutions · 2298 PASS · 2 FAIL · 2 h 39** (journaux `/tmp/rw-lot-jIH6Fh`, 153 verdicts sur 153 attendus). **+16 PASS et un FAIL de moins qu'hier, après 44 commits.** **AUCUN des deux écarts n'est une régression du portage** — et les deux sont des défauts de SUITE : (a) `go-page-ssh-flux` rend **0/0 sur les deux cibles** — `deployment.log` a été **rotationné** en `.log.1` le 27/08 à 22:43 par E-196, *notre correctif*, et la suite **suppose un fichier qu'elle ne crée pas** : elle meurt avant d'imprimer son tampon. **Couplage caché qu'un LOT complet existe pour révéler — le correctif était juste, la suite était fragile, et rien ne les reliait.** (b) `go-page-graylog-g1` 21/2, **rejouée seule au repos : 21/2 aussi**, donc l'hypothèse de la charge est RÉFUTÉE ; la page est **mesurée saine** (sonde jetable : le clic ouvre le panneau, champ à 45 px, contrat DOM intact, zéro erreur JS) — le défaut est dans l'enchaînement de la suite, **non isolé et non prétendu tel**. `go-socle-navigation` passe à **65** (`api_docs` a basculé `legacy` → `route`, garde `'sa'`, +1 au rôle 3 ; 26+6, total 32 inchangé) : `REF_LARAVEL` inscrit. **Le LOT s'est arrêté À SA FIN, pas sur un incident — le gel des conteneurs est LEVÉ le 2026-08-28 à 16:45 CEST.** Remesure : `./scripts/rejouer-lot.sh` puis **attendre un PID enregistré** (`pgrep -f "[r]ejouer-lot"` a rendu trois PID dont le shell qui demandait : la classe de caractères est nécessaire, **pas suffisante**).
