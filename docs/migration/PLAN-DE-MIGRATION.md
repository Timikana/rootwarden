...
| 7 | `services/` | 631 (2 fichiers) | 1 | **INVENTORIÉ `v1.37.92` — `MODULE-SERVICES.md`**, trois sous-lots S1 à S3, **S1 PORTÉ `v1.37.94`**, **S2 CARACTÉRISÉ `v1.37.95`** (12 legacy, 0 FAIL) — reste le port de S2, puis S3. **E-149 : les huit routes n'ont NI rôle NI permission — seule la page est gardée.** Réel dans le code, non exploitable par aucun compte existant : à un `UPDATE` de l'être |
