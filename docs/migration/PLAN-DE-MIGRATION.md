...
Dernière mise à jour : **2026-09-01** (16:10 CEST), version `1.38.111`. Le chantier tourne désormais à **huit sessions** (le DSI délégué inclus) à propriété disjointe — table au §10 de `PROTOCOLE-SESSIONS.md`.
...
| LOT de tests E2E | ⚠ **LES DEUX FAIL NOMMÉS SONT RÉGLÉS ; LA LIGNE DE BASE NE L'EST PAS.** La base **153 exécutions · 2298 PASS · 2 FAIL** date du **2026-08-28 16:45**, et depuis : **67 commits, 12 fichiers SERVIS touchés, dont 3 GLOBAUX** — `Middleware/ChangementMotDePasseExige.php` (appliqué **sur le groupe**, `web.php:93`), `public/css/rw.css` (le socle) et `layouts/portail.blade.php` (le gabarit). **Ces trois-là touchent les 153 exécutions**, donc *un rejeu ciblé sur « ce qui a bougé » serait une illusion de proportion*, et **le total est INCONNU depuis le 2026-08-28**. Les deux FAIL nommés sont éteints et mesurés : `go-page-ssh-flux` **10/8** (`assureJournal()`) et `go-page-graylog-g1` **28/27, 0 FAIL** (E-241b, `--rw-defilement-marge: 88px`) — leurs références portent déjà ces valeurs. **LOT COMPLET RELANCÉ le 2026-09-01 vers 16:10 sur arbitrage du Lead** : le redémarrage n'a pas de date, donc différer la mesure derrière lui n'était pas « plus tard » mais « aucune ligne de base pour une durée inconnue » — et sans témoin d'avant, le premier LOT d'après-redémarrage n'aurait pu séparer *régression du portage* de *effet du redémarrage*. ⚠ **Premier LOT contenant une suite qui ÉCRIT en base** (`go-page-mot-de-passe`) **et premier avec l'abattage** : sa détection est testée sur journal forgé, **sa sortie de boucle ne l'est pas** — `go-page-mot-de-passe` est jouée en tête pour qu'un abattage à tort se voie à la minute 2 et non à la minute 130. Remesure : `./scripts/rejouer-lot.sh`, **attendre un PID enregistré**, et **conclure sur le JOURNAL**.
