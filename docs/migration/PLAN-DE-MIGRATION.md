...
| LOT de tests E2E | **NOUVELLE LIGNE DE BASE, 2026-08-28 : 153 exécutions · 2298 PASS · 2 FAIL · 2 h 39** (journaux `/tmp/rw-lot-jIH6Fh`, 153 verdicts sur 153 attendus). **+16 PASS et un FAIL de moins qu'hier, après 44 commits.** **AUCUN des deux écarts n'est une régression du portage** — et les deux sont des défauts de SUITE : (a) `go-page-ssh-flux` rend **0/0 sur les deux cibles** — `deployment.log` a été **rotationné** en `.log.1` le 27/08 à 22:43 par E-196, *notre correctif*, et la suite **suppose un fichier qu'elle ne crée pas** : elle meurt avant d'imprimer son tampon. **Couplage caché qu'un LOT complet existe pour révéler — le correctif était juste, la suite était fragile, et rien ne les reliait.** (b) `go-page-graylog-g1` 21/2 — ⚠ **J'AI INSCRIT « la page est mesurée saine » ET C'ÉTAIT FAUX** (voir **E-241**). C'est une **régression réelle du portage** : après un enregistrement de configuration refusé, **les onglets cessent de répondre** ; le legacy rend **27/0**. Ma note reposait sur une sonde qui mesurait le clic **au chargement**, pas *après* la séquence qui casse — et je l'ai recopiée d'un compte rendu qui s'annonçait lui-même comme non isolé. **La correction de la session 7 n'a pas fait disparaître le FAIL : elle l'a NOMMÉ**, ce qui était son objet. `go-socle-navigation` passe à **65** (`api_docs` a basculé `legacy` → `route`, garde `'sa'`, +1 au rôle 3 ; 26+6, total 32 inchangé) : `REF_LARAVEL` inscrit. **Le LOT s'est arrêté À SA FIN, pas sur un incident — le gel des conteneurs est LEVÉ le 2026-08-28 à 16:45 CEST.** Remesure : `./scripts/rejouer-lot.sh` puis **attendre un PID enregistré** (`pgrep -f "[r]ejouer-lot"` a rendu trois PID dont le shell qui demandait : la classe de caractères est nécessaire, **pas suffisante**).
| tests backend | **TRANCHE le 2026-08-28 a 16:28 CEST — `566 passed · 1 skipped · 1 xfailed`** (pytest, conteneur `rootwarden_python`) et **`277 passed · 888 assertions`** (`php artisan test`, `rootwarden_laravel`). Commit `c282fbe`, **verifie au registre par le Lead** — pas remesure : relancer `pytest` aurait coute a la session 7 sa sonde E-241 en cours. *Verifier un registre plutot que refaire une mesure est parfois le geste juste, et c'est le cas quand refaire coute a quelqu'un d'autre.* ⚠ **IL N'Y A JAMAIS EU DE CONTRADICTION** : `509` (27/08 13:18) et `549` (28/08 08:49) sont **tous deux justes, a onze heures d'ecart** — progression `375 → 464 → 483 → 509 → 540 → 543 → 549 → 551 → 552 → 566`, chaque palier date dans son commit. **Le seul chiffre faux etait `462`, qui comptait des `def test_`** — des FONCTIONS : une fonction parametree a six jeux rend six cas. *Un compte de fonctions n'est pas un compte de cas, et c'est ce repere qui rendait tous les ecarts inexplicables.* **`566` reste vrai exactement le temps du gel d'ecriture backend** : il cessera de l'etre a la premiere ecriture de la session 4. `sudo -n docker exec rootwarden_python sh -c "cd /app && python -m pytest -q"` — **et jamais pendant qu'une autre session tient le banc.**
| écarts de parité documentés | **226** — numérotés jusqu'à **E-237** ; dix numéros, **E-23 à E-32**, n'ont jamais servi : *le dernier numéro n'est pas un compte.* **Neufs du 2026-08-28** : **E-234, une CONSOLE D'API sur `documentation.php`** — champ libre, méthode, corps JSON, dès le rôle 1 sur la page et **aucun `checkPermission`** ; **E-235/235b/235c, le rempart `ADMIN_SEULEMENT` manquant — et l'ensemble DÉRIVÉ est 38 espaces là où quatre listes successives disaient 2, 3, 7** ; **E-236, deux gardes qui NE S'ORDONNENT PAS** — un rôle 2 sans `can_audit_ssh` ne peut pas ouvrir la page `ssh-audit` **et peut appeler `scan-all`** ; **E-237, `/wazuh/uninstall` : verdict corrigé, état persisté laissé, et le correctif évident est faux dans l'autre sens.** `grep -c '^## E-' docs/migration/PARITE.md`
