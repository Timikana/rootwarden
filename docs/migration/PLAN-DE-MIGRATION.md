...
| | |
|---|---|
| entrées de menu portées | **24 sur 33**, et le total se RECONSTITUE — mesuré le 2026-08-26 en faisant lire `Navigation::SECTIONS` **par PHP lui-même** : `24 route + 9 legacy + 0 ni l'un ni l'autre = 33` (D9a, D9b, B1 et S1 ont fait basculer `sudo_policies`, `sftp_policies`, `bashrc` et `services`). Un premier comptage à l'expression régulière avait rendu 32, en manquant `wazuh` — voir §8. Restent en `legacy` : `iptables`, `fail2ban`, `ssh_audit`, **`wazuh`**, `groups`, `remote_users`, `platform_key`, `documentation`, `api_docs` |
...
| écarts de parité documentés | **140** — numérotés jusqu'à **E-150** : dix numéros, **E-23 à E-32**, n'ont jamais été utilisés. Le dernier numéro n'est donc pas un compte. `grep -c '^## E-' docs/migration/PARITE.md` |
...
| 7 | `services/` | 631 (2 fichiers) | 1 | **INVENTORIÉ `v1.37.92` — `MODULE-SERVICES.md`**, trois sous-lots S1 à S3, **S1 et S2 PORTÉS** (`v1.37.94`, `v1.37.96`), **S3 CARACTÉRISÉ `v1.37.97`** (12 legacy, 0 FAIL) — reste le port de S3. **Le banc est un conteneur SANS systemd** : le rendu d'un tableau peuplé n'est mesuré sur aucune cible. **E-149 : les huit routes n'ont NI rôle NI permission — seule la page est gardée.** Réel dans le code, non exploitable par aucun compte existant : à un `UPDATE` de l'être |
...
> **Cinq correctifs backend attendent désormais le même arbitrage** — E-142, E-144, E-147, E-149, E-150 —
