...
| écarts de parité documentés | **269** — identifiants dédupliqués, numérotés jusqu'à **E-280**, **treize numéros jamais servis** (E-23→E-32, E-186, E-196, E-204). ⚠ **E-278 : la commande inscrite ici comptait des TITRES, pas des écarts** — 273 contre 266, et l'erreur *grandissait avec la tenue du registre* (un écart clos ou amendé reçoit un second titre). Les « dix jamais servis » venaient d'une soustraction entre deux grandeurs différentes ; par différence d'ensembles il y en a treize. Commande corrigée au §2. Neufs : **E-278** (mon instrument) · **E-279** (`/ssh-audit/trends` promise par `openapi.yaml`, appelée par personne) · **E-280** (⚠ une planification de scan SSH vise tout le parc par REPLI, et **le repli EN SERVICE n'a aucun filtre** — archivées comprises). |
