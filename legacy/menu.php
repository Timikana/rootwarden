...
<aside id="sidebar" class="fixed left-0 top-0 h-full w-56 bg-gray-900 dark:bg-gray-950 border-r border-gray-800 z-50 flex flex-col transition-all duration-200 hidden lg:flex">
    ...
    <nav class="flex-1 px-3 space-y-1 overflow-y-auto">
        ...
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8444', '/') . '/maintenance', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>', t('nav.maintenance'), t('nav.tip_maintenance')) ?>
        <?php /* PORTE vers Laravel (2026-08-18) : page archivee dans
                 legacy/_deprecated/. L'entree renvoie au nouveau portail. */ ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8444', '/') . '/approbations', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>', t('nav.approvals'), t('nav.tip_approvals')) ?>
        ...
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8444', '/') . '/derive-config', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>', t('nav.drift'), t('nav.tip_drift')) ?>
        <?php endif; ?>
        <?php endif; ?>
...
<div id="mobile-drawer" class="lg:hidden fixed inset-0 z-50 -translate-x-full transition-transform duration-200">
    ...
    <div class="relative w-56 h-full bg-gray-900 overflow-y-auto">
        ...
        <nav class="px-3 py-3 space-y-1">
            ...
            <?php if (($perms['can_manage_services'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8444', '/') ?>/services" title="<?= t('nav.tip_services') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.services') ?></a><?php endif; ?>
