...
<aside id="sidebar" class="fixed left-0 top-0 h-full w-56 bg-gray-900 dark:bg-gray-950 border-r border-gray-800 z-50 flex flex-col transition-all duration-200 hidden lg:flex">
    ...
    <div class="px-4 py-4 border-b border-gray-800">
        <a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/accueil" class="flex items-center gap-2">
    ...
    <nav class="flex-1 px-3 space-y-1 overflow-y-auto">
        ...
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/mises-a-jour', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>', t('nav.updates'), t('nav.tip_updates')) ?>
        <?php endif; ?>

        <?php if (($perms['can_manage_iptables'] ?? false) || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/pare-feu', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>', t('nav.iptables'), t('nav.tip_iptables')) ?>
        <?php endif; ?>

        <?php if (($perms['can_manage_fail2ban'] ?? false) || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/fail2ban', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"/></svg>', t('nav.fail2ban'), t('nav.tip_fail2ban')) ?>
        <?php endif; ?>

        <?php if (($perms['can_manage_services'] ?? false) || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/services', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>', t('nav.services'), t('nav.tip_services')) ?>
        <?php endif; ?>

        <?php if (($perms['can_audit_ssh'] ?? false) || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/audit-ssh', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>', t('nav.ssh_audit'), t('nav.tip_ssh_audit')) ?>
        <?php endif; ?>

        <?php if (($perms['can_manage_supervision'] ?? false) || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/supervision', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>', t('nav.supervision'), t('nav.tip_supervision')) ?>
        <?php endif; ?>

        <?php if (($perms['can_manage_bashrc'] ?? false) || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/bashrc', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>', t('nav.bashrc'), t('nav.tip_bashrc')) ?>
        <?php endif; ?>

        <?php if (($perms['can_manage_graylog'] ?? false) || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/graylog', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 10h16M4 14h10M4 18h10"/></svg>', t('nav.graylog'), t('nav.tip_graylog')) ?>
        <?php endif; ?>

        <?php if (feature_enabled('wazuh') && (($perms['can_manage_wazuh'] ?? false) || $isSA)): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/wazuh', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 2L3 7v6c0 5.25 3.75 9.75 9 11 5.25-1.25 9-5.75 9-11V7l-9-5z"/></svg>', t('nav.wazuh'), t('nav.tip_wazuh')) ?>
        <?php endif; ?>

        <?php if (($perms['can_scan_cve'] ?? false) || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/scan-cve', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>', t('nav.cve_scan'), t('nav.tip_cve_scan')) ?>
        <?php endif; ?>
        <?php if ($isAdmin || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/docker', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13h4v4H5zM10 13h4v4h-4zM15 13h4v4h-4zM10 8h4v4h-4zM3 17h18v3H3z"/></svg>', t('nav.docker'), t('nav.tip_docker')) ?>
        <?php endif; ?>
        ...
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/comptes', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>', t('nav.admin'), t('nav.tip_admin')) ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/taches', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/></svg>', t('nav.tasks'), t('nav.tip_tasks')) ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/groupes', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m6-1.13a4 4 0 10-4-4 4 4 0 004 4zm6 0a3 3 0 10-2.83-4M7 11a3 3 0 10-2.83-4"/></svg>', t('nav.groups'), t('nav.tip_groups')) ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/maintenance', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>', t('nav.maintenance'), t('nav.tip_maintenance')) ?>
        <?php /* PORTE vers Laravel (2026-08-18) : page archivee dans
                 legacy/_deprecated/. L'entree renvoie au nouveau portail. */ ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/approbations', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>', t('nav.approvals'), t('nav.tip_approvals')) ?>
        <?php /* PORTE vers Laravel (2026-08-18) : la page a ete archivee dans
                 legacy/_deprecated/. L'entree renvoie desormais au nouveau
                 portail plutot que de rendre 404 — un menu qui mene nulle part
                 est le defaut qu'on corrige, pas celui qu'on installe. */ ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/journal-commandes', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>', t('nav.commandlog'), t('nav.tip_commandlog')) ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/chatops', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 4v-4z"/></svg>', t('nav.chatops'), t('nav.tip_chatops')) ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/tickets', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5v2m0 4v2m0 4v2M5 5h14a2 2 0 012 2v3a2 2 0 000 4v3a2 2 0 01-2 2H5a2 2 0 01-2-2v-3a2 2 0 000-4V7a2 2 0 012-2z"/></svg>', t('nav.tickets'), t('nav.tip_tickets')) ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/recherche', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>', t('nav.search'), t('nav.tip_search')) ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/journal-audit', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>', t('nav.audit_log'), t('nav.tip_audit_log')) ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/sauvegardes', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 7v10c0 2 1 3 3 3h10c2 0 3-1 3-3V7M4 7c0-2 1-3 3-3h10c2 0 3 1 3 3M4 7h16M9 12h6"/></svg>', t('nav.backups'), t('nav.tip_backups')) ?>
        <?php endif; ?>
        <?php if (($perms['can_manage_remote_users'] ?? false) || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/comptes-distants', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>', t('nav.remote_users'), t('nav.tip_remote_users')) ?>
        <?php endif; ?>
        <?php if (($perms['can_manage_platform_key'] ?? false) || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/cle-plateforme', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>', t('nav.platform_key'), t('nav.tip_platform_key')) ?>
        <?php endif; ?>
        <?php if ($isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/politiques', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg>', t('nav.sudo_policies'), t('nav.tip_sudo_policies')) ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/acces-sftp', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16V4m0 0L3 8m4-4l4 4m6 0v12m0 0l4-4m-4 4l-4-4"/></svg>', t('nav.sftp_policies'), t('nav.tip_sftp_policies')) ?>
        <?php endif; ?>
        <?php if (($perms['can_view_compliance'] ?? false) || $isSA): ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/rapport-conformite', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>', t('nav.compliance'), t('nav.tip_compliance')) ?>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/derive-config', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>', t('nav.drift'), t('nav.tip_drift')) ?>
        <?php endif; ?>
        <?php endif; ?>

        <div class="text-[10px] text-gray-500 uppercase tracking-wider px-3 py-2 mt-3"><?= t('nav.section_other') ?></div>
        <?= $sideLink(rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') . '/documentation', '<svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>', t('nav.documentation'), t('nav.tip_documentation')) ?>
        <?php /*
            L'ENTREE « console d'API » A ETE RETIREE, ET C'EST UNE DECISION.

            Elle visait `/api/docs.php`, archive. Mais `E-234` a tranche que la
            console NE SE PORTE PAS — `DOSSIER-10` le dit : « le portage n'a pas
            de console ». Il n'y a donc PAS d'equivalent a pointer.

            La rebasculer vers `cles-api` enverrait qui cherche une console vers
            une page de CLES : deux choses differentes, et le lecteur croirait
            avoir trouve. **Une capacite REFUSEE n'a pas d'equivalent, et lui en
            inventer un est pire que le lien mort.**

            Retiree plutot que laissee morte : un menu qui mene nulle part est le
            defaut qu'on corrige, pas celui qu'on installe (:135 de ce fichier).
        */ ?>
    </nav>
    ...
    <div class="px-3 py-3 border-t border-gray-800">
        <a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/profil" class="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-gray-800 transition-colors">
        ...
        <div class="flex items-center gap-1 mt-2 px-3">
            ...
            <a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/deconnexion" class="flex-1 text-center text-xs text-red-400 hover:text-red-300 hover:bg-red-900/30 px-2 py-1.5 rounded-lg transition-colors"><?= t('nav.logout') ?></a>
        </div>
...
<div class="lg:hidden fixed top-0 left-0 right-0 h-12 bg-gray-900 dark:bg-gray-950 border-b border-gray-800 z-50 flex items-center justify-between px-4">
    ...
    <a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/profil" class="w-7 h-7 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-bold"><?= strtoupper(substr($username, 0, 1)) ?></a>
</div>
...
<div id="mobile-drawer" class="lg:hidden fixed inset-0 z-50 -translate-x-full transition-transform duration-200">
    ...
    <div class="relative w-56 h-full bg-gray-900 overflow-y-auto">
        ...
        <nav class="px-3 py-3 space-y-1">
            <a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/accueil" title="<?= t('nav.tip_dashboard') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.dashboard') ?></a>
            <?php if (($perms['can_deploy_keys'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/cles-ssh" title="<?= t('nav.tip_ssh_keys') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.ssh_keys') ?></a><?php endif; ?>
            <?php if (($perms['can_update_linux'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/mises-a-jour" title="<?= t('nav.tip_updates') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.updates') ?></a><?php endif; ?>
            <?php if (($perms['can_manage_iptables'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/pare-feu" title="<?= t('nav.tip_iptables') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.iptables') ?></a><?php endif; ?>
            <?php if (($perms['can_manage_fail2ban'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/fail2ban" title="<?= t('nav.tip_fail2ban') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.fail2ban') ?></a><?php endif; ?>
            <?php if (($perms['can_manage_services'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/services" title="<?= t('nav.tip_services') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.services') ?></a><?php endif; ?>
            <?php if (($perms['can_audit_ssh'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/audit-ssh" title="<?= t('nav.tip_ssh_audit') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.ssh_audit') ?></a><?php endif; ?>
            <?php if (($perms['can_manage_supervision'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/supervision" title="<?= t('nav.tip_supervision') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.supervision') ?></a><?php endif; ?>
            <?php if (($perms['can_manage_bashrc'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/bashrc" title="<?= t('nav.tip_bashrc') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.bashrc') ?></a><?php endif; ?>
            <?php if (($perms['can_manage_graylog'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/graylog" title="<?= t('nav.tip_graylog') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.graylog') ?></a><?php endif; ?>
            <?php if (feature_enabled('wazuh') && (($perms['can_manage_wazuh'] ?? false) || $isSA)): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/wazuh" title="<?= t('nav.tip_wazuh') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.wazuh') ?></a><?php endif; ?>
            <?php if (($perms['can_scan_cve'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/scan-cve" title="<?= t('nav.tip_cve_scan') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.cve_scan') ?></a><?php endif; ?>
            <?php if ($hasAdminSection): ?>
            <hr class="border-gray-800 my-2">
            <?php if (($perms['can_admin_portal'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/comptes" title="<?= t('nav.tip_admin') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.admin') ?></a><?php endif; ?>
            <?php if (($perms['can_manage_remote_users'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/comptes-distants" title="<?= t('nav.tip_remote_users') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.remote_users') ?></a><?php endif; ?>
            <?php if (($perms['can_manage_platform_key'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/cle-plateforme" title="<?= t('nav.tip_platform_key') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.platform_key') ?></a><?php endif; ?>
            <?php if (($perms['can_view_compliance'] ?? false) || $isSA): ?><a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/rapport-conformite" title="<?= t('nav.tip_compliance') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.compliance') ?></a><?php endif; ?>
            <?php endif; ?>
            <hr class="border-gray-800 my-2">
            <a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/documentation" title="<?= t('nav.tip_documentation') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.documentation') ?></a>
            <a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/profil" title="<?= t('nav.tip_profile') ?>" class="block px-3 py-2 rounded-lg text-sm text-gray-300 hover:bg-gray-800"><?= t('nav.profile') ?></a>
            <a href="<?= rtrim(getenv('LARAVEL_URL') ?: 'http://localhost:8080', '/') ?>/deconnexion" class="block px-3 py-2 rounded-lg text-sm text-red-400 hover:bg-red-900/30"><?= t('nav.logout') ?></a>
