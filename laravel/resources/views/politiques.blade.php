                    ...
                    @foreach ($historique as $h)
                        <tr>
                            ...
                            {{--
                                ...
                                Ce bloc offrait un LIEN vers `adm/server_user_sudo.php`, page
                                ...
                                ⚠ Elle reecrit un sudoers en root. Elle est derriere le
                            ...
                            <td class="rw-tableau__actions">
                                @if (in_array($h->status, ['applied', 'superseded'], true))
                                    <button type="button" class="rw-bouton rw-bouton--danger rw-bouton--minuscule"
                                            data-rw="politique-rollback"
                                            data-deploiement="{{ $h->id }}"
                                            title="{{ __('politiques.rollback_texte') }}">{{ __('politiques.rollback') }}</button>
                                @endif
                            </td>
                        </tr>
                    @endforeach
...
@endif
    ...
    <div class="rw-panneau-decision" data-rw="politiques-panneau-stepup" hidden>
        ...
        <label class="rw-champ">
            ...
            <input type="text" inputmode="numeric" maxlength="6" autocomplete="one-time-code"
                   class="rw-saisie rw-saisie--code" data-rw="politiques-stepup-code">
        </label>
        <div class="rw-panneau-decision__actions">
            <button type="button" class="rw-bouton rw-bouton--discret"
                    data-rw="politiques-stepup-annuler">{{ __('step_up.panneau_annuler') }}</button>
            <button type="button" class="rw-bouton"
                    data-rw="politiques-stepup-valider">{{ __('step_up.panneau_valider') }}</button>
        </div>
    </div>

    {{-- Les libelles du defi, depuis le catalogue PARTAGE : `step-up.js` sert
         deux pages, et un libelle recopie par page divergerait. --}}
    <script id="politiques-stepup-libelles" type="application/json">@json(__('step_up'))</script>
