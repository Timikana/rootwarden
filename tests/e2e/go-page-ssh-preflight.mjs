...
try {
    ...
    /*
     * `verifiePortage` d'ICI transmet `d` TEL QUEL a un `verifie(l, ok, d)` qui
     * l'imprime sur un PASS comme sur un FAIL — a la difference de celui de
     * `go-fail2ban-f2`, qui fait `ok ? '' : d`. L'explication d'echec
     * s'affichait donc sur la ligne verte du portage, qui separe pourtant bien
     * les deux gestes. Le detail se garde par la condition.
     */
    const okSeparable = commandes.verificationSeparee && commandes.zoneRapport;
    verifiePortage('le constat est SEPARABLE du deploiement', okSeparable,
        okSeparable
            ? 'un bouton de verification seule, et sa zone de rapport'
